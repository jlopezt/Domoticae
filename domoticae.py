...
def ficheroConfiguracion(usuario,nombreDevice,servicio):
    ...
    if request.method == 'POST': 
        ...
        print('El fichero se guardara en ' + nombreFichero)
...
def handle_mqtt_message(client, userdata, message):    
    try:
        ...
        print("------------------error--------------")
        print(e)  
        print("Topic que genera el error: ", message.topic)
        print("Mensaje que genera el error: ", message.payload.decode("utf-8"))
        db.rollback()
        print("Error en la consulta")
        return KO   
